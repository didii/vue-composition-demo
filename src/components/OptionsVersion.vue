<template>
  <div id="options">
    <modal v-if="showNewFileModal" @onAccept="onAcceptModal($event)" @onCancel="onCancelModal()"></modal>
    <h1>Folder manager (Options)</h1>
    <div class="row">
      <div class="col-12">
        <input v-model="filter" type="text" class="form-control" placeholder="Search..." />
      </div>
    </div>
    <hr />
    <div class="row">
      <div class="col-12">
        <div class="folder-view">
          <h2>
            <i class="far fa-folder-open"></i>
            {{rootFolder.name}}
          </h2>
          <folder
            :folderInfo="filteredFolderInfo"
            @onUpload="onUploadFile($event)"
            @onDeleteFile="onDeleteFile($event)"
            @onDeleteFolder="onDeleteFolder($event)"
          ></folder>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { Vue, Component, Watch } from "vue-property-decorator";
import Folder from "./Folder.vue";
import Modal from "./Modal.vue";
import { FolderInfo, data, FileInfo } from "@/models";

@Component({
  components: {
    Folder,
    Modal,
  }
})
export default class OptionsVersion extends Vue {
  private rootFolder: FolderInfo = { name: "Loading", folders: [], files: [] };
  private filter: string = "";
  private selectedFolder: FolderInfo | null = null;
  private showNewFileModal: boolean = false;

  public mounted() {
    let stored = localStorage.getItem("folders");
    if (stored) {
      this.rootFolder = JSON.parse(stored);
    } else {
      this.rootFolder = data;
    }
  }

  @Watch("rootFolder", { deep: true })
  public onRootFolderWatch(newValue: FolderInfo) {
    localStorage.setItem("folders", JSON.stringify(newValue));
  }

  public get filteredFolderInfo(): FolderInfo {
    return this.doFilter(this.rootFolder, this.filter);
  }

  public onUploadFile(folder: FolderInfo) {
    this.showNewFileModal = true;
    this.selectedFolder = this.findFolder(this.rootFolder, folder);
  }

  public onDeleteFile(file: FileInfo) {
    let parentFolder = this.findParentFolderOfFile(this.rootFolder, file);
    if (parentFolder) {
      parentFolder.files = parentFolder.files.filter(x => x.name !== file.name);
    }
  }

  public onDeleteFolder(folder: FolderInfo) {
    let found = this.findParentFolderOfFolder(this.rootFolder, folder);
    if (found) {
      found.folders = found.folders.filter(x => x.name !== folder.name);
    }
  }

  public onAcceptModal(filename: string) {
    if (this.selectedFolder) {
      this.selectedFolder.files.push({ name: filename });
      this.selectedFolder.files.sort((a, b) => a.name.localeCompare(b.name));
    }
    this.showNewFileModal = false;
  }

  public onCancelModal() {
    this.selectedFolder = null;
    this.showNewFileModal = false;
  }

  private doFilter(folder: FolderInfo, filter: string): FolderInfo {
    return {
      name: folder.name,
      folders: folder.folders
        .map(x => this.doFilter(x, filter))
        .filter(
          x =>
            x.name.includes(filter) ||
            x.folders.length > 0 ||
            x.files.length > 0
        ),
      files: folder.files.filter(x =>
        x.name.toLowerCase().includes(filter.toLowerCase())
      )
    };
  }

  private findFolder(root: FolderInfo, toFind: FolderInfo): FolderInfo | null {
    if (root.name === toFind.name) {
      return root;
    }
    for (let folder of root.folders) {
      let found = this.findFolder(folder, toFind);
      if (found) {
        return found;
      }
    }
    return null;
  }

  private findParentFolderOfFolder(root: FolderInfo, toFind: FolderInfo): FolderInfo | null {
    if (root.folders.some(x => x.name === toFind.name)) {
      return root;
    }
    for (let folder of root.folders) {
      let found = this.findParentFolderOfFolder(folder, toFind);
      if (found) {
        return found;
      }
    }
    return null;
  }

  private findParentFolderOfFile(root: FolderInfo, toFind: FileInfo): FolderInfo | null {
    for (let file of root.files) {
      if (file.name === toFind.name) {
        return root;
      }
    }
    for (let folder of root.folders) {
      let found = this.findParentFolderOfFile(folder, toFind);
      if (found) {
        return found;
      }
    }
    return null;
  }
}
</script>
